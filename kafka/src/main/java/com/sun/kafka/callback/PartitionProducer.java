...
//		 prop.put("retries", 1);//重试次数
		 ...
		 MyPartitionProducer my = new MyPartitionProducer();
		 my.partition("test", my, null, my, null, null);
		 
		 prop.put(ProducerConfig.PARTITIONER_CLASS_CONFIG, "com.sun.kafka.callback.MyPartitionProducer");
...
//		 值，也就是常说的 round-robin 算法。轮询（2，0，1）依次给每个分区加数据，这样可以使数据均匀分布
		 ...
		 for(int i=0; i<10; i++){
			 ProducerRecord record = new ProducerRecord<String,String>("test",Integer.toString(i));
